	...
	<div id="dvContents">
			<div style = "margin-left:px;">
				...
				<?php
					...
					$select = "SELECT *,SUM(nodays) AS total1 FROM employee, vacation_log
									WHERE employee.emp_id = vacation_log.emp_id AND employee.emp_id='$_GET[id]'";
					$qry=mysql_query($select);
					$rec = mysql_fetch_array($qry);
					
			$sql="select SUM(nodays) AS nod1 From vacation_log
					GROUP BY emp_id";
			...
			$value3 = $cred - $value2;
				...
				<table border="0" cellspacing="10" cellpadding="10" width="600px" style="margin-left:100px">
					<tr>
						...
						<td>
							Date: &nbsp <strong><?php echo $rec['vdate']; ?></strong>
						</td>
					</tr>
					<tr>
						<td>
					...
					<tr>
						<td>
						Total Leave: &nbsp <strong><?php echo $rec['total1'];?></strong>
						</td>
						<td>
						Days Left: &nbsp <strong><?php echo $value3;?></strong>
						</td>
					</tr>
				</table>
				...
				<?php
					require ("../conn.php");   // Include the database class
					$select = "SELECT * FROM employee, vacation_log
									WHERE employee.emp_id = vacation_log.emp_id AND employee.emp_id='$_GET[id]'";
					$qry=mysql_query($select);
				?>
				<?php
				...
				?>
					...
					<table border="0" cellspacing="10" cellpadding="10" width="600px" style="margin-left:100px">
						...
						<tr>
							<td>
							Effective Date: &nbsp <strong><?php echo $rec1['sdate'];?></strong> &nbsp to &nbsp <strong><?php echo $rec1['eddate'];?></strong>
							</td>
						</tr>
						<tr>
							<td>
							Total Days Leave: &nbsp <strong><?php echo $rec1['nodays'];?></strong><hr/>
							</td>
						</tr>
				<?php
					}
				?>
						...
						<div style="margin-left:120px">
							<a href="vl_reports.php"><input type = "button" value="Back" style = "background: #transparent; width:60px;height:30px;"/></a>
